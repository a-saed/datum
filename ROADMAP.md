...
- **`datum-cli mcp`** — one command starts Postgres, `datum-server`, and the MCP bridge together for AI agents, reusing `datum-cli dev`'s existing Postgres resolution. The `query` MCP tool now caps results at `--max-rows` (default 1000) with truncation metadata instead of returning unbounded result sets.
